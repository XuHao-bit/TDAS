...
def id_storing_movielens_cold_only(max_count=20):
    storing_path = 'data_processed'
    dataset = 'movielens'
    ...
    if not os.path.exists('{}/{}/user_state_ids.p'.format(storing_path, dataset)):
        ...
        # user_statistics
        user_iter_num = ratings_dat['user_id'].value_counts()
        user_df = user_iter_num.reset_index()
        user_df.rename(columns={'index':'user_id','user_id':'num'}, inplace=True)

        split_num = user_df['num'][round(0.2*user_df.shape[0])]
        warm_user = user_df[user_df.num>split_num]
        cold_user = user_df[user_df.num<=split_num]
        print(f"max interaction num:{warm_user.iloc[0]['num']}, split num:{split_num}, min num:{cold_user.iloc[-1]['num']}")
        
        user_cold_list = cold_user['user_id'].to_list()
        new_df = pd.DataFrame()

        for u_id in tqdm(user_cold_list):
            u_info = sorted_users.loc[sorted_users.user_id == u_id].reset_index(drop=True)
            # u_count = len(u_info)
        ...
        # item_statistics
        item_ids = new_df.item_id.unique()
